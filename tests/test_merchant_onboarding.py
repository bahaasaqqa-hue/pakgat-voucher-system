...
class MerchantOnboardingTests(unittest.TestCase):
    ...
    def test_unknown_phone_can_verify_before_merchant_is_created(self):
        with patch.object(onboarding.secrets, "randbelow", return_value=123456), patch.object(
            onboarding, "_send_whatsloop_text", return_value=(True, "HTTP 200")
        ) as send:
        ...
        application = self.db.scalar(
            select(onboarding.MerchantOnboardingApplication).where(
                onboarding.MerchantOnboardingApplication.merchant_id == merchant.id
            )
        )
    ...
    def test_documents_are_open_multi_upload_metadata_not_fixed_types(self):
        ...
        with patch.dict(os.environ, {"MERCHANT_DOCUMENT_ROOT": self.tmp.name}):
            first = onboarding.store_onboarding_document(
                self.db,
                merchant,
            ...
            second = onboarding.store_onboarding_document(
                self.db,
                merchant,
                filename="شهادة-البنك.png",
    ...
    def test_submit_requires_declaration_and_at_least_one_document(self):
        ...
        with self.assertRaises(ValueError):
            onboarding.submit_onboarding(self.db, merchant, declaration_accepted=False)
        with self.assertRaises(ValueError):
            onboarding.submit_onboarding(self.db, merchant, declaration_accepted=True)

        with patch.dict(os.environ, {"MERCHANT_DOCUMENT_ROOT": self.tmp.name}):
            onboarding.store_onboarding_document(
                self.db,
                merchant,
    ...
    def test_sadq_signature_moves_application_to_pakgat_review_not_active(self):
        ...
        contract = finance.MerchantContract(
            ...
            agreement_number="PKG-MA-2026-08-0999",
            status="sadq_pending",
            sadq_document_id="sadq-doc",
            sadq_transaction_id="sadq-env",
        )
    ...
    def test_pakgat_can_approve_only_after_sadq_signed(self):
        ...
        contract = finance.MerchantContract(
            merchant_id=merchant.id,
            agreement_number="PKG-MA-2026-08-1000",
            status="sadq_pending",
        )
        self.db.add(contract)
        self.db.commit()
        with self.assertRaises(ValueError):
            onboarding.approve_signed_onboarding(self.db, contract)
