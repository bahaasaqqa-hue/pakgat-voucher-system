...
BACKUP_DIR="/opt/pakgat-repair-backups/jood-merchant-followup-v2-${STAMP}"
WORKTREE="/tmp/pakgat-jood-merchant-followup-v2-${STAMP}"
LIVE_PATCHED=0
RESTART_ATTEMPTED=0
...
run_suite_signature() {
  ...
  local logfile="$2"
  local sigfile="$3"
  LABEL="$label" LOGFILE="$logfile" SIGFILE="$sigfile" "$APP_DIR/.venv/bin/python" - <<'PY'
import json
import os
import unittest
...
for test, _tb in result.failures:
    signature.append({"kind": "failure", "id": test.id()})
for test, _tb in result.errors:
    signature.append({"kind": "error", "id": test.id()})
signature.sort(key=lambda row: (row["id"], row["kind"]))
with open(sigfile, "w", encoding="utf-8") as fh:
    json.dump(signature, fh, ensure_ascii=False, sort_keys=True, indent=2)

print(f"{label}_TESTS_RUN={result.testsRun}")
print(f"{label}_FAILURES={len(result.failures)}")
print(f"{label}_ERRORS={len(result.errors)}")
print(f"{label}_SIGNATURE=" + json.dumps(signature, ensure_ascii=False, sort_keys=True))
PY
}
...
echo "===== 2. PRE-FLIGHT BASELINE ON EXACT LIVE SOURCE — NO CHANGES ====="
cleanup_worktree
...
run_suite_signature "BASELINE" "$WORKTREE/baseline-suite.log" "$WORKTREE/baseline-signature.json"

echo "===== 3. PATCH PREVIEW + TARGETED TESTS ====="
...
echo "===== 4. FULL REGRESSION ON PATCHED COPY ====="
run_suite_signature "PATCHED" "$WORKTREE/patched-suite.log" "$WORKTREE/patched-signature.json"

if ! cmp -s "$WORKTREE/baseline-signature.json" "$WORKTREE/patched-signature.json"; then
  echo "ABORT: patched suite changed the unrelated failure/error signature."
  echo "--- BASELINE SIGNATURE ---"
  cat "$WORKTREE/baseline-signature.json"
  echo
  echo "--- PATCHED SIGNATURE ---"
  cat "$WORKTREE/patched-signature.json"
  false
fi

echo "DIFFERENTIAL_REGRESSION_SIGNATURE=PASS"
cat "$WORKTREE/patched-signature.json"
echo

echo "===== 5. BACKUP LIVE INBOUND ====="
mkdir -p "$BACKUP_DIR"
chmod 700 "$BACKUP_DIR"
cp -a "$APP_DIR/app/whatsloop_inbound.py" "$BACKUP_DIR/whatsloop_inbound.py"
cp -a "$WORKTREE/baseline-suite.log" "$BACKUP_DIR/baseline-suite.log"
cp -a "$WORKTREE/patched-suite.log" "$BACKUP_DIR/patched-suite.log"
cp -a "$WORKTREE/baseline-signature.json" "$BACKUP_DIR/baseline-signature.json"
cp -a "$WORKTREE/patched-signature.json" "$BACKUP_DIR/patched-signature.json"
git -C "$APP_DIR" status --short > "$BACKUP_DIR/git-status-before.txt"
echo "BACKUP_DIR=$BACKUP_DIR"

echo "===== 6. APPLY ONLY MERCHANT FALLBACK CHANGE ====="
...
grep -q 'إذا تقصد كيف تتم آلية التعاون مع بكجات' "$APP_DIR/app/whatsloop_inbound.py"
! grep -q 'أرسل لي اسم النشاط والمدينة ونوع الخدمات' "$APP_DIR/app/whatsloop_inbound.py"
grep -q 'def open_handoff_blocks_current_outreach' "$APP_DIR/app/whatsloop_inbound.py"
"$APP_DIR/.venv/bin/python" -m py_compile "$APP_DIR/app/whatsloop_inbound.py"

echo "===== 7. RESTART + HEALTH POLL ====="
RESTART_ATTEMPTED=1
...
echo "===== 8. POST-RESTART RUNTIME CHECK ====="
load_prod_env
.venv/bin/python - <<'PY'
