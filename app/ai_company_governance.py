...
Implements the Blueprint execution levels (AUTO / APPROVAL / ONLY_CEO) without
performing sensitive commercial actions automatically.  It also generates
compact daily/weekly/monthly CEO briefs from data already available in the
Google Data Hub.
"""
...
def generate_ceo_brief(db: Session, period: str = "daily", save: bool = False) -> str:
    ...
    high_alerts = int(
        db.scalar(
            select(func.count(ai_company.CompanyAlert.id)).where(
...
def governance_page(request: Request, db: Session = Depends(core.get_db)):
    ...
    approval_rows = "".join(
        ...
        f"<td>{('<form method=\'post\' action=\'/admin/company/governance/%d/approve\' style=\'display:inline\'><button class=\'btn btn-blue\'>موافقة</button></form> <form method=\'post\' action=\'/admin/company/governance/%d/reject\' style=\'display:inline\'><button class=\'btn btn-muted\'>رفض</button></form>' % (a.id, a.id)) if a.status == 'pending' else '—'}</td>"
        "</tr>"
        for a in approvals
    ...
    <main class='wrap' style='padding:28px 0 48px'>
      ...
      <section class='card' style='padding:22px;margin-bottom:18px'>
        <h2>قائمة الموافقات</h2><div class='table-wrap'><table><thead><tr><th>الأولوية</th><th>المستوى</th><th>المصدر</th><th>الإجراء</th><th>الحالة</th><th>القرار</th></tr></thead><tbody>{approval_rows}</tbody></table></div>
      </section>
      <section class='card' style='padding:22px'>
        <h2>سجل القرارات</h2><div class='table-wrap'><table><thead><tr><th>العنوان</th><th>القرار</th><th>السبب</th><th>التاريخ</th></tr></thead><tbody>{decision_rows}</tbody></table></div>
      </section>
    </main>
    """
...
def ceo_brief_page(request: Request, period: str = "daily", db: Session = Depends(core.get_db)):
    ...
    <main class='wrap' style='padding:28px 0 48px'>
      ...
      <section class='card' style='padding:24px;margin-bottom:18px'><div style='line-height:2;font-size:17px'>{content}</div><form method='post' action='/admin/company/brief/save?period={core.esc(period)}'><button class='btn btn-blue' style='margin-top:16px'>حفظ هذا الملخص</button></form></section>
