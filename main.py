...
app = FastAPI(title="Pakgat Voucher System", version="2.0", lifespan=lifespan)
...
VOUCHER_PRODUCT_IDS = {v.strip() for v in env("VOUCHER_PRODUCT_IDS", "782332771").split(",") if v.strip()}
...
def page_shell(title: str, body: str, admin: bool = False) -> str:
    nav = ""
    if admin:
        nav = f'<div style="display:flex;gap:8px;flex-wrap:wrap"><a class="btn btn-muted" href="/admin">لوحة الإدارة</a><a class="btn btn-muted" href="/admin/vouchers/new">قسيمة جديدة</a><form method="post" action="/admin/logout" style="margin:0"><button class="btn btn-danger" type="submit">تسجيل الخروج</button></form></div>'
...
def update_voucher_status(voucher: Voucher, db: Session) -> str:
    ...
    if voucher.status == "active" and expires < now_utc():
        voucher.status = "expired"; db.commit(); db.refresh(voucher)
...
@app.get("/")
def home():
    return {"status": "running", "service": "Pakgat Voucher System", "version": "2.0", "admin": BASE_URL + "/admin/login", "database": "connected"}
